...
#define LEVEL(file,title,...) static const char *message_##file[] = {__VA_ARGS__};
    LEVEL_LIST
#undef LEVEL
...
static const u8 lives_per_age[] = {
    2, 2, 2, 2
};
static const u32 turn_time_per_age[] = {
    300, 350, 350, 600
};
static const u32 sprite_per_age[] = {
    SPRITE_hero, SPRITE_hero2, SPRITE_hero3, SPRITE_hero4,
};
static const u32 hurt_per_age[] = {
    SOUND_hurt1, SOUND_hurt2, SOUND_hurt3, SOUND_hurt4,
};
static const float age_time[] = {
    0.f, 8.f, 17.f, 40.f
};
static const float max_age = 60.f;
static const u32 start_years = 12;
static const u32 end_years = 105;
static const u32 num_ages = array_count(age_time);
static const float normal_age_rate = 1.5f;
...
static u32 add_obj(Object obj, GameData *gd)
{
    obj.prev_pos = obj.pos;
    s32 count = sb_count(gd->objs);
    u8 found = 0;
    ...
    if(obj.flags & OBJ_SOLID){
        ...
        gd->obj_map[a] = (s32)obj.index;
    }
    return obj.index;
}

static void destroy_obj(u32 index, GameData *gd)
{
    ...
    obj->flags = 0;
    for(u32 i = 0;i < map_size_x*map_size_y;i++){
        if(gd->obj_map[i] == (s32)index){
            gd->obj_map[i] = -1;
        }
    }
}
...
{
    s32 i = gd->obj_map[x+y*map_size_x];
    if(i != -1){
        return &gd->objs[i];
    }
    return 0;
...
static u8 is_valid_location(u32 x, u32 y, Object *self, GameData *gd)
{
...
/*static Object *get_obj_at_location(u32 x, u32 y, GameData *gd)
{
    Object *obj = 0;
    s32 index = gd->obj_map[x+y*map_size_x];
    if(index != -1){
        obj = gd->objs + index;
    }
    return obj;
...
{
    Object obj = {0,OBJ_player,{x,y},SPRITE_hero,
        OBJ_SOLID|OBJ_VISIBLE|OBJ_ALIVE|OBJ_MOVING,0};
    gd->player_id = add_obj(obj,gd);
}
static void update_hero(Object *obj, GameData *gd, UNUSED float dt)
{
...
{
    Object obj = {0,OBJ_wall,{x,y},SPRITE_wall,
        OBJ_DEFAULT};
    add_obj(obj,gd);
}
void spawn_crystal(u32 x, u32 y, GameData *gd)
{
    Object obj = {0,OBJ_hourglass,{x,y},SPRITE_crystal,
        OBJ_DEFAULT};
    add_obj(obj,gd);
}
...
static void spawn_wall_switch(u8 phase, u8 switch_id, u32 sprite, u32 x, u32 y, GameData *gd)
{
    Switchable* s = malloc(sizeof(Switchable));
    s->phase = phase;
    s->switch_id = switch_id;
    Object obj = {s,OBJ_wall_switch,{x,y},sprite,
        OBJ_DEFAULT};
    add_obj(obj,gd);
}
...
static void update_wall_switch(Object *obj, GameData *gd, UNUSED float dt)
{
...
static void free_wall_switch(Object *obj)
{
    free(obj->data);
}
...
void spawn_skeleton(u32 x, u32 y, GameData *gd)
{
    ...
    d->stun_time = 0.f;
    Object obj = {d,OBJ_skeleton,{x,y},SPRITE_skeleton,
        OBJ_VISIBLE|OBJ_ALIVE|OBJ_MOVING};
    u32 index = add_obj(obj,gd);
    u32 a = obj.pos.x + obj.pos.y*map_size_x;
    gd->obj_map[a] = (s32)index;
}
static void free_enemy(Object *obj)
{
    free(obj->data);
}
static u8 is_solid_skeleton(UNUSED Object *a, Object *b, GameData *gd)
{
    switch(b->type){
        case OBJ_player:
...
static u8 collide_skeleton(UNUSED Object *a, Object *b, GameData *gd)
{
...
static void update_skeleton(Object *obj, UNUSED GameData *gd, UNUSED float dt)
{
...
{
    {
        ...
        d->open = 0;
        Object obj = {d,OBJ_gate,{x,y},SPRITE_gate_closed,
            OBJ_VISIBLE|OBJ_ALIVE};
        u32 a = obj.pos.x + obj.pos.y*map_size_x;
        gd->obj_map[a] = (s32)add_obj(obj,gd);
    }
    {
        GateData *d = malloc(sizeof(GateData));
        d->open = 0;
        Object obj = {d,OBJ_gate,{(x-1),y},SPRITE_gate_closed,
            OBJ_ALIVE};
        u32 a = obj.pos.x + obj.pos.y*map_size_x;
        gd->obj_map[a] = (s32)add_obj(obj,gd);
    }
}
static void free_gate(Object *obj)
{
    free(obj->data);
}
static u8 is_solid_gate(Object *a, Object *b, UNUSED GameData *gd)
{
    GateData *d = a->data;
    return d->open == 0 || b->type != OBJ_player;
}
static u8 collide_gate(Object *a, Object *b, GameData *gd)
{
    GateData *d = a->data;
...
static void update_gate(Object *obj, GameData *gd, UNUSED float dt)
{
    GateData *d = obj->data;
    if(!d->open){
        u8 ankh = 0;
        for(int i=0;i<sb_count(gd->objs);i++){
            Object *o = gd->objs+i;
            if(o->type == OBJ_ankh && o->flags & OBJ_ALIVE){
                ankh = 1;
            }
        }
        if(!ankh){
...
{
    Object obj = {0,OBJ_ankh,{x,y},SPRITE_ankh,
        OBJ_VISIBLE|OBJ_ALIVE};
    u32 a = obj.pos.x + obj.pos.y*map_size_x;
    gd->obj_map[a] = (s32)add_obj(obj,gd);
}
static void update_ankh(Object *obj, GameData *gd, UNUSED float dt)
{
    obj->offset.y = sinf(gd->time*2.f)*.2f;
}
static u8 is_solid_ankh(UNUSED Object *a, Object *b,UNUSED GameData *gd)
{
    return b->type != OBJ_player;
}
static u8 collide_ankh(Object *a, Object *b, GameData *gd)
{
...
{
    Object obj = {0,OBJ_hourglass,{x,y},SPRITE_hourglass,
        OBJ_VISIBLE|OBJ_ALIVE};
    u32 a = obj.pos.x + obj.pos.y*map_size_x;
    gd->obj_map[a] = (s32)add_obj(obj,gd);
}
static void update_hourglass(Object *obj, GameData *gd, UNUSED float dt)
{
    obj->offset.y = sinf(gd->time*2.f)*.2f;
}
static u8 is_solid_hourglass(UNUSED Object *a, Object *b, UNUSED GameData *gd)
{
    return b->type != OBJ_player;
}
static u8 collide_hourglass(Object *a, Object *b, GameData *gd)
{
    if(b->type == OBJ_player){
...
static ObjectFuncs obj_funcs[] = {
    ...
    {update_skeleton,is_solid_skeleton,free_enemy,collide_skeleton},
    {update_gate,is_solid_gate,free_gate,collide_gate},
    ...
    switch(gd->state){
        ...
        case STATE_LEVEL_FINISHED:
            if(gd->message_time > 0.999f){
                gd->current_level = min_u8(num_levels-1,gd->current_level+1);
                load_level(gd);
            }
            break;
...
static void mouse_down(float x, UNUSED float y, GameData *gd)
{   
    if(x < .5f){
...
static void input_game(GameStateData *data,SDL_Event event)
{
    ...
    switch(event.type){
                ...
                if(!event.key.repeat && event.key.type == SDL_KEYDOWN){
                    switch(event.key.keysym.sym){
                        ...
                        case SDLK_EQUALS:
                            ...
                            break;
                        case SDLK_MINUS:
                            gd->current_level = max_u8(0,gd->current_level-1);
                            load_level(gd);
                            break;
                        case SDLK_a:
                            turn_time = 100;
            ...
            case SDL_FINGERDOWN: {
                float x,y;
                pixels2screen((u32)event.tfinger.x,(u32)event.tfinger.y,&x,&y);
                mouse_down(x,y,gd);
            }break;
            case SDL_MOUSEBUTTONDOWN: {
                float x,y;
                pixels2screen((u32)event.button.x,(u32)event.button.y,&x,&y);
                mouse_down(x,y,gd);
            }break;
        default: break;
    }
...
static void draw_game(GameStateData *data)
{
    ...
    for(u32 yy=0;yy<map_size_y;yy++){
        for(u32 xx=0;xx<map_size_x;xx++){
            s32 i = gd->obj_map[xx+yy*map_size_x];
            if(i!=-1){
                Object *obj = &gd->objs[i];
                if(obj->flags & (OBJ_VISIBLE)){
                    ...
                    if(obj->type == OBJ_player){
                        ...
                        if(gd->blink_time > 0.1f){
                            alpha = .6f + 0.3f*sinf(gd->time*13.f);
                        }
    ...
    // Draw hud
    for(u32 i=0;i<lives_per_age[gd->age];i++){
        if(gd->lives > i){
    ...
    if(levels[gd->current_level].id != LEVEL_level1 &&
      levels[gd->current_level].id != LEVEL_level0){
        ...
        draw_text(menu_font,font_color,buffer, (s32)x, 0,.5f,-.3f,cam.scale*1.5f);
    }
    screen2pixels(1.f,0.f,&x,&y);
    {
        SDL_Color font_color = {159,75,33,255};
        draw_text(menu_font,font_color,level_text,(s32)x,(s32)y,1.5f,0.f,cam.scale*1.5f);
    }
    {
        SDL_Color font_color = {181,108,16,255};
        draw_text(menu_font,font_color,level_name,(s32)x,(s32)y,1.5f,-1.6f,cam.scale*.7f);
    }
...
static void get_direction_xy(u8 dir, s32 *x, s32 *y)
{
    *x = 0;
    *y = 0;
    switch(dir){
...
static void update_direction(Object *obj, GameData *gd)
{
    ...
    u32 target_x = (u32)((s32)obj->pos.x+x);
    u32 target_y = (u32)((s32)obj->pos.y+y);
    if(!is_valid_location(target_x,target_y,obj,gd)){
        obj->direction = (obj->direction + 1) % 4;
        get_direction_xy(obj->direction,&x,&y);
        target_x = (u32)((s32)obj->pos.x+x);
        target_y = (u32)((s32)obj->pos.y+y);
    }
    if(!is_valid_location(target_x,target_y,obj,gd)){
...
void load_level(GameData *gd)
{
    ...
    gd->map = malloc(sizeof(u32)*(size_t)(m.w*m.h));
    gd->obj_map = malloc(sizeof(s32)*(size_t)(m.w*m.h));
    //layer 0 - tiles
    for(int y=0;y<m.h;y++){
        for(int x=0;x<m.w;x++){
            gd->map[x+y*m.w] = (u32)m.tiles[x+y*m.w];
    ...
    for(s32 y=0;y<m.h;y++){
        for(s32 x=0;x<m.w;x++){
            s16 s = t[x+y*m.w];
            if(s != -1){
                Spawner spawner = spawners[(u32)s-num_tiles];
                spawner.spawn_func((u32)x,(u32)y,gd);
            }
        }
    }
    delete_map(m);
